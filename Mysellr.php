 ...
 *
 * @package		MySellr
 * @author		MySellr Dev Team
 * @copyright	Copyright (c) 2011, MySellr.
 * @license		http://mysellr.com/license.html
 * @link		http://mysellr.com
 * @since		Version 1.0
 * @filesource
 */
...
/**
 * MySellr API Client Class
 *
 * Handle request to MySellr API
 *
 * @package		MySellr
...
if (!function_exists('curl_init')) {
  throw new Exception('MySellr needs the CURL PHP extension.');
}

if (!function_exists('json_decode')) {
  throw new Exception('MySellr needs the JSON PHP extension.');
}
...
class Mysellr
{
  ...
  /**
   * The ID of the MySellr user, or 0 if the user is logged out.
   */
  protected $user;
  ...
  /**
   * Initialize a MySellr Application.
   *
   * The configuration:
  ...
  {
    return implode('_', array('mysellr', $this->getAppId(), $key));
  }
   ...
   * if the MySellr user is not connected.
   *
   ...
   *
   * @return Integer the id of the connected MySellr user, or 0
   ...
   * seemingly legitimate.  It relies on MySellr API
   * to retreive user information and then extract
   * the user ID.
   *
   * @return Integer returns the UID of the MySellr user, or
   * 0 if the MySellr user could not be determined.
   */
   ...
   * (previously generated from market.mysellr.com on behalf of
  ...
  {
    return 'mysellrcsrf_'.$this->getAppId();
  }
  ...
  /**
   * Returns the Current URL, stripping it of known MySellr parameters that should
   * not persist.
   *
